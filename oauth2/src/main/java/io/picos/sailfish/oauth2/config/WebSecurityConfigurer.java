...
import io.picos.sailfish.oauth2.support.security.UserDetailsAuthenticationProviderImpl;
import io.picos.sailfish.oauth2.support.security.UserDetailsServiceImpl;
import io.picos.sailfish.oauth2.support.security.rest.AuthenticationFailureHandlerRestImpl;
import io.picos.sailfish.oauth2.support.security.rest.AuthenticationSuccessHandlerRestImpl;
import io.picos.sailfish.oauth2.support.security.rest.LogoutSuccessHandlerRestImpl;
...
public class WebSecurityConfigurer extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests().antMatchers("/login").permitAll()
            .and()
            .requestMatchers().antMatchers("/oauth/authorize").and().authorizeRequests().anyRequest().authenticated()
            .and()
            .formLogin().permitAll();
